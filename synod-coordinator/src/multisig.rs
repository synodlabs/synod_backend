...
use crate::{AppState, stellar, auth::AuthUser};
...
pub async fn get_multisig_setup(
    ...
    .fetch_optional(&state.db)
    .await?
    .ok_or(AppError::NotFound("No wallet connected to this treasury".into()))?;
    ...
    if coordinator_pubkey.is_empty() {
        return Err(AppError::Internal(anyhow::anyhow!("Coordinator pubkey not configured")));
    }
...
pub async fn revoke_multisig(
    ...
    .fetch_optional(&state.db)
    .await?
    .ok_or(AppError::NotFound("Wallet not found in this treasury".into()))?;

    // 2. Check for bypass (clean up only)
    if payload.xdr == "OFF_CHAIN_BYPASS" {
        info!("Bypass requested: Cleaning up database record for wallet {}", wallet.wallet_address);
        sqlx::query!(
    ...
    let mut envelope = TransactionEnvelope::from_xdr(&raw_env, crate::stellar::next_xdr::Limits::none())
    ...
    let TransactionEnvelope::Tx(v1_env) = &mut envelope else {
        return Err(AppError::Internal(anyhow::anyhow!("Only V1 Transaction Envelopes are supported")));
    };

    // Construct the Hash for verification and signing
    use sha2::{Sha256, Digest};
    let network_id = Sha256::digest(state.config.stellar.network_passphrase.as_bytes());
    
    let mut hasher = Sha256::new();
    hasher.update(&network_id);
    hasher.update(&[0, 0, 0, 2]); // EnvelopeTypeTx (V1)
    
    let tx_xdr = v1_env.tx.to_xdr(crate::stellar::next_xdr::Limits::none())
        .map_err(|e| AppError::Internal(anyhow::anyhow!("TX encoding error: {}", e)))?;
    hasher.update(&tx_xdr);
    let hash = hasher.finalize();

    // CRYPTO SELF-TEST: Verify the user's signature against our calculated hash
    // This ensures our hashing logic is identical to the wallet's
    let user_pk_bytes = crate::stellar::decode_stellar_address(&wallet.wallet_address)?;
    let user_verifying_key = ed25519_dalek::VerifyingKey::from_bytes(&user_pk_bytes).map_err(|e| AppError::Internal(anyhow::anyhow!("Invalid VerifyingKey: {}", e)))?;
    
    let mut verified = false;
    for sig in v1_env.signatures.iter() {
        let sig_bytes: &[u8] = &sig.signature.0;
        let dalek_sig = ed25519_dalek::Signature::from_slice(sig_bytes).map_err(|_| AppError::Internal(anyhow::anyhow!("Invalid signature format")))?;
        if user_verifying_key.verify_strict(&hash, &dalek_sig).is_ok() {
            verified = true;
            break;
        }
    }
    ...
    if !verified {
        error!("Security Hash Mismatch: Backend calculated hash {} does not match user signature. Check network passphrase or XDR version.", hex::encode(hash));
        return Err(AppError::Internal(anyhow::anyhow!("Security Error: Signature verification failed. Ensure you are on the correct network (Testnet) and try again.")));
    }

    info!("User signature verified. Appending Coordinator signature...");

    // Sign the hash
    let secret_bytes = crate::stellar::decode_secret_key(&state.config.stellar.coordinator_secret_key)?;
    let signing_key = ed25519_dalek::SigningKey::from_bytes(&secret_bytes);
    let signature_bytes = <ed25519_dalek::SigningKey as ed25519_dalek::Signer<ed25519_dalek::Signature>>::sign(&signing_key, &hash).to_bytes();
    ...
    sigs.push(DecoratedSignature {
        ...
        signature: Signature(signature_bytes.to_vec().try_into().map_err(|_| AppError::Internal(anyhow::anyhow!("Invalid signature length")))?)
    });
    v1_env.signatures = sigs.try_into().map_err(|_| AppError::Internal(anyhow::anyhow!("Too many signatures")))?;

    // 3. Submit to Horizon
    let final_xdr = envelope.to_xdr(Limits::none())
        .map_err(|e| AppError::Internal(anyhow::anyhow!("Final XDR encoding error: {}", e)))?;
    let final_xdr_base64 = BASE64.encode(&final_xdr);

    info!("Submitting co-signed revocation to Stellar for wallet: {}", wallet.wallet_address);
    let horizon_url = &state.config.stellar.horizon_url;
    let client = reqwest::Client::new();
    let horizon_res = client.post(format!("{}/transactions", horizon_url))
    ...
    if !horizon_res.status().is_success() {
        ...
        return Err(AppError::Internal(anyhow::anyhow!("On-chain revocation failed: {}. Ensure your account meets signing thresholds.", err_body)));
    }
