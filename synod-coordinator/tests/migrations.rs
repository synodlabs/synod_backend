...
async fn migrations_apply_cleanly(pool: PgPool) {
    ...
    assert!(
        table_names.contains(&"user_passkeys"),
        "Missing table: user_passkeys"
    );
    assert!(
        table_names.contains(&"wallet_connections"),
        "Missing table: wallet_connections"
    );
    assert!(
        table_names.contains(&"treasuries"),
        "Missing table: treasuries"
    );
    assert!(
        table_names.contains(&"treasury_wallets"),
        "Missing table: treasury_wallets"
    );
    assert!(
        table_names.contains(&"constitution_history"),
        "Missing table: constitution_history"
    );
    assert!(
        table_names.contains(&"agent_slots"),
        "Missing table: agent_slots"
    );
    assert!(
        table_names.contains(&"permit_groups"),
        "Missing table: permit_groups"
    );
...
async fn permits_table_has_indexes(pool: PgPool) {
    ...
    assert!(
        idx_names.contains(&"idx_permits_agent_status"),
        "Missing index: idx_permits_agent_status"
    );
    assert!(
        idx_names.contains(&"idx_permits_treasury_status"),
        "Missing index: idx_permits_treasury_status"
    );
}
