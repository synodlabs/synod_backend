...
async fn test_phase_5_constitution_flow() {
    ...
    let rollback_res = client
        .post(format!(
            "{}/v1/treasuries/{}/constitution/rollback/1",
            base_url, treasury_id
        ))
        .header("Authorization", format!("Bearer {}", token))
        .send()
...
async fn test_phase_5_governance_signing() {
    ...
    let v2_content = ConstitutionContent {
        ...
        agent_wallet_rules: vec![AgentWalletRule {
            agent_id: Uuid::new_v4(),
    ...
    let sign_res = client
        .post(format!(
            "{}/v1/treasuries/{}/proposals/{}/sign",
            base_url, treasury_id, proposal_id
        ))
        .header("Authorization", format!("Bearer {}", token))
    ...
    let history_res = client
        .get(format!(
            "{}/v1/treasuries/{}/constitution/history",
            base_url, treasury_id
        ))
        .header("Authorization", format!("Bearer {}", token))
        .send()
